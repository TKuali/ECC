...
# Match ECC's invisible-character checks. These are not meaningful source text.
INVISIBLE_RANGES = (
    (0x200B, 0x200D), (0x202A, 0x202E), (0x2060, 0x2064),
    (0x2066, 0x2069), (0xFE00, 0xFE0F), (0xE0100, 0xE01EF),
    (0xE0000, 0xE007F),
)
INVISIBLE_POINTS = {0xFEFF, 0x180E, 0x115F, 0x1160, 0x3164}
# Decorations present in the pinned upstream README, not a general emoji filter.
SNAPSHOT_DECORATIONS = {0x1F9D9, 0x2642, 0x1F4D6, 0x1F970, 0x1F631}


def normalize_source(text: str) -> str:
    """Remove hidden controls and known decorations; preserve readable source text."""
    normalized = ''.join(
        char for char in text
        if ord(char) not in INVISIBLE_POINTS | SNAPSHOT_DECORATIONS
        and not any(start <= ord(char) <= end for start, end in INVISIBLE_RANGES)
    )
    return '\n'.join(line.rstrip(' \t\r') for line in normalized.split('\n'))


def validate_refresh(catalog: dict, output_dir: Path) -> None:
    """Reject empty or suspiciously incomplete input before writing any package files."""
    count = len(catalog['tools'])
    if count == 0:
        raise ValueError('Refusing empty catalog refresh; inspect the source before retrying.')
    ...
    try:
        ...
        raise ValueError('Refusing to replace an unreadable existing catalog; inspect it manually.') from exc
    # Count is a conservative completeness signal, not proof of source integrity.
    if count * 5 < len(tools) * 4:
        raise ValueError(
            f'Refusing catalog shrink from {len(tools)} to {count} tools (more than 20%); '
            'inspect the upstream source and review any intentional replacement manually.'
        )
...
def main() -> int:
    ...
    try:
        ...
        validate_refresh(catalog, out)
        (out / 'source').mkdir(parents=True, exist_ok=True)
        # The digest is computed over UTF-8 bytes. Avoid Windows newline conversion.
        (out / 'source' / 'awesome-osint-README.md').write_bytes(text.encode('utf-8'))
